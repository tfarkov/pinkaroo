...
Used to protect the cron endpoint that runs MLS sync.

### Steps

1. Generate a secret (e.g. `openssl rand -base64 24`).
2. Add to `.env`:
   ```env
   CRON_SECRET=your_cron_secret
   ```
3. When calling the cron job (e.g. from a scheduler), send:  
